...
# fi
mkdir -p $log_dir
default_model_list=("Qwen/Qwen2.5-1.5B-Instruct" "meta-llama/Llama-3.1-8B-Instruct" "openai-community/gpt2")
extra_model_list=("mistralai/Mistral-7B-Instruct-v0.3" "microsoft/Phi-3-mini-4k-instruct" "EleutherAI/gpt-j-6b" "baichuan-inc/Baichuan2-7B-Chat" "meta-llama/Llama-2-13b-hf" "facebook/opt-6.7b")

if [[ $specified_model_list == "default" ]]; then
  model_list=("${default_model_list[@]}")
elif [[ $specified_model_list == "all" ]]; then
  model_list=("${default_model_list[@]}" "${extra_model_list[@]}")
else
  IFS=' ' read -r -a model_list <<< "$specified_model_list"
fi
